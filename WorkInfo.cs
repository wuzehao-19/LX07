...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public int GetCount(int[] activityIds)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) frorm workinfo where activityId in @activityIds";
        ...
        public int Add(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert info workinfo(workId,workName,workPicture,uploadTime,workIntroduction,workVerify,userName,activityId,recommend,recommendTime)" +
                    "values(@workIdm,@workName,@workPicture,@uploadTime,@workIntroduction,@workVerify,@userName,@activtiyId,@recommend,@recommendTime";
                sql += "select @@identity";
